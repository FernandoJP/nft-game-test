...
contract LipToken is ERC721, Ownable {
    ...
    //transfer the token to owner's wallet, token is generated by minting
    function withdraw() external payable onlyOwner {
        address payable _owner = payable(owner());
        _owner.transfer(address(this).balance);
    }

    function getMintedToken() external view onlyOwner returns (uint256) {
        return address(this).balance;
    }

    //Creation
    function _createLip(string memory _name) internal {
        uint8 randRaririty = uint8(_createRandomNum(100));
        uint256 randDna = _createRandomNum(10**16);
        Lip memory newLip = Lip(_name, COUNTER, randDna, 1, randRaririty);
        lips.push(newLip);
